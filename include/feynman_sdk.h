...
#define PREVIEW_FRAME_DATA_MAX_SIZE	(ONE_PACKET_DATA_MAX_SIZE - sizeof(FrameHeader))
	...
	typedef enum {
		FEYNMAN_IR_IMAGE_LEFT_VI = 0x00,  /**< ir sensor left, YUV,未经标定校准 0  */
		FEYNMAN_IR_IMAGE_RIGHT_VI, /**< ir sensor right, YUV，未经标定校准 1  */
		FEYNMAN_IR_IMAGE_LEFT_VPSS, /**< ir sensor left, YUV，标定校准后 2  */
		FEYNMAN_IR_IMAGE_RIGHT_VPSS, /**< ir sensor right, YUV，标定校准后 3  */
		FEYNMAN_IR_IAMGE_DUAL,  /**< ir sensor dual，feynman项目此类型无效 4  */
		FEYNMAN_RGB_IMAGE_SINGLE,/**< rgb sensor single, 待定 5  */
		FEYNMAN_RGB_IMAGE_LEFT, /**< rgb sensor left，feynman项目此类型无效 6  */
		FEYNMAN_RGB_IMAGE_RIGHT,/**< rgb sensor right，feynman项目此类型无效 7  */
		FEYNMAN_RGB_IMAGE_DUAL, /**< rgb sensorr dual，feynman项目此类型无效 8  */
		FEYNMAN_DISPARITYIMAGE,  /**< depth的视差图，16bit RAW 9  */
		FEYNMAN_DEPTH_IMAGE,     /**< depth的视差图的RAW数据，16bit RAW 10  */
		FEYNMAN_DEPTH_IMAGE_LEFT_RAW, /**< 参与depth的左图RAW数据，8bit RAW 11  */
		FEYNMAN_DEPTH_IMAGE_RIGHT_RAW, /**< 参与depth的右图RAW数据，8bit RAW 12  */
	}FEYNMAN_IMAGE_SUB_TYPE;
...
#pragma pack(1)
	...
	*/
	typedef struct
	{
		uint8_t sn[16];       /**< device sn of feynman,sn[15]为偶数：非广角相机， 为奇数：广角相机 */
	}s_feynman_device_sn;
	...
	*/
	typedef struct
	{
		uint32_t crruent_value;  /**< projector status of feynman device,0-关闭投射器 大于0-打开投射器并设置电流值 */
	...
	*/
	typedef struct
	{
		int status; /**< bit0:-0 stop transfer, “1-start transfer,bit1:ACC,bit2:GYO,bit3:MANG.(0:do not transfer,1:transfer)”，目前是全部上传，所开是1关是0 */
	...
	*/
	typedef struct
	{
		...
		int is_new_format;  /**< 0-旧的标定文件格式  1-新的标定文件格式 */


		char camera_mode[16]; /**< 相机模型 */
		int camera_number;/**<camera number */

		float left_sensor_focus[2]; /**< [0] - X方向焦距, [1] - Y方向焦距 */
		float left_sensor_photocenter[2];/**< [0] - 光心X坐标  [1]-光心Y坐标 */

		float right_sensor_focus[2]; /**< [0] - X方向焦距, [1] - Y方向焦距 */
		float right_sensor_photocenter[2];/**< [0] - 光心X坐标  [1]-光心Y坐标 */

		float rgb_sensor_focus[2];/**< [0] - X方向焦距, [1] - Y方向焦距 */
		float rgb_sensor_photocenter[2];/**< [0] - 光心X坐标  [1]-光心Y坐标 */

		float left2right_extern_param[12]; /**< 对于新的标定格式，基线长度位于left2right_extern_param[9]，对于旧的标定格式，基线长度位于left2right_extern_param[3] */
	...
	*/
	typedef struct
	{
		uint32_t data_number;/**< 本次传输多少组数据，每组数据及排序： acc：xyz三个数据，  gyo：xyz三个数据； mang：三个数据； temp：一个数据 */
		int16_t data[0];/**< data */
	}s_feynman_imu_data;
	...
	*/
	typedef struct
	{
		uint32_t packet_numbers; /**< 一共多少包数据 */
		uint32_t curr_packet_numbers;/**< 当前是第几包数据 */
		uint32_t data_len;/**< 数据长度 */
		char     path[256];/**< 文件路径 例如Feynman程序 "/nextvpu/bin/arm" */
		char     name[100];/**< 文件名字，例如Feynman程序 "feynman" */
		char     data[0];/**< 传输的数据 */
	...
	*/
	typedef struct
	{
		...
		float cpu_temperaure;  /**< CPU温度，单位摄氏度 */
		float projector_temperaure[2];/**< 0-projector1 data, 1-projector2 data */
		s_feynman_device_id device_id; /**< 设备ID号，与FEYNMAN_COMMAND_GET_DEVICE_ID_COMMAND的结构一致 */
		s_feynman_device_sn device_sn;  /**< 设备SN号，与FEYNMAN_COMMAND_GET_DEVICE_SN_COMMAND的结构一致 */
		s_feynman_sensor_resolution_fps_infor fps;/**< sensor分辨率、帧率信息 */
		int depth_ir_transfer;/**< 0-stop transfer, 1-start transfer， 仅在运行depth时有效 */
		int depth_depth_transfer;   /**<0-stop transfer, 1-start transfer， 仅在运行depth时有效 */
		int imu_transfer;/**< 0-stop transfer, 1-start transfer */
		int image_freq;  /**< 图像接收的帧率 */
